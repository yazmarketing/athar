...
4. **Configure env** — copy `.env.example` to `.env.local` and fill in:
   ...
   - `GEMINI_API_KEY` (server only) — one key serves Nano Banana, Nano Banana 2, and Nano Banana Pro in the image-model picker. Without it they are still listed but fail at generate time.
