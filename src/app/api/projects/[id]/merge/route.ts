...
import { mergeProjects } from "@/lib/entity-merges";
...
export async function POST(
  ...
  try {
    ...
    if (!UUID_RE.test(id) || typeof body.targetId !== "string" || !UUID_RE.test(body.targetId)) {
      return NextResponse.json({ error: "Valid source and destination projects are required" }, { status: 400 });
    }
    const result = await mergeProjects(id, body.targetId);
    await logAudit({
    ...
    const status = message.includes("not found") ? 404 : message.includes("different") || message.includes("same client") || message.includes("required") ? 400 : 500;
