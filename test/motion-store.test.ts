...
import { createProject, getProject, listProjects, locked, projectRoot, queueNative, queueSourceStudy, saveProject, userRoot } from "../src/lib/motion/store";
const users:string[]=[];
...
describe("native project persistence",()=>{
 ...
 it("returns overview snapshots without reconciling active projects inline",async()=>{const u=user(),p=await createProject(u,"Queued","create");p.status="queued";p.commandId=randomUUID();p.commandAt=Date.now()-130000;await saveProject(u,p);const [snapshot]=await listProjects(u);expect(snapshot).toMatchObject({id:p.id,status:"queued"});});
