...
function Preview() {
  ...
  return <div style={{ display: "flex", height: "100dvh" }}>
    ...
    <main className="flex min-w-0 flex-1 flex-col">{selection && <div role="status" className="shrink-0 border-b border-border bg-card p-4 text-sm">{selection}<button className="ml-4 underline" onClick={() => setSelection("")}>Dismiss</button></div>}{view === "Explore" ? <StudioHome firstName="" searchRef={searchRef} query={query} onQueryChange={setQuery} onClearProject={() => {}} onOpen={(destination) => setSelection(`Open ${destination}`)} onRecipe={choose} onVideoModel={(tier) => setSelection(`Video model selected: ${tier}`)} recentContent={<div className="rounded-xl border border-dashed border-border p-10 text-center text-sm text-muted-foreground">Your generated images and videos appear here.</div>} /> : <div className="overflow-y-auto p-6"><VideoExplore onSelect={choose} compact={view === "Video"} /></div>}</main>
  </div>;
}
